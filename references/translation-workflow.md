...
**Ownership:** Evaluation clones (swap text, scale tiers) in **Phases 1–2.6** when `executionPath: "figma"` + `reportOnly: false`. Official `{outputSectionName}` matrix in **Phase 4 only**. **Never** call `use_figma` when `reportOnly: true`.
