...
0. CONFIG — Phase 0 validation (always): reject Your Project Name on every path; non-empty customLocales when custom pack; locales × screenVariants ≥ 1; INVALID combo executionPath figma + reportOnly true (suggest prototype+reportOnly true or figma+reportOnly false); reject Figma placeholders when reportOnly false OR figma/both. STOP CONFIG_INVALID on fail.
